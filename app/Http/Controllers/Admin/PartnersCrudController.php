...
class PartnersCrudController extends CrudController
{
    ...
    /**
     * Show a partner with tabs: data, contacts and documents
     */
    public function show($id)
    {
        $this->crud->hasAccessOrFail('show');

        $partner = $this->crud->model::with('user')->findOrFail($id);

        return view('vendor.backpack.partners.show', [
            'partner' => $partner,
        ]);
    }
    ...
    public function setup()
    {
        ...
        CRUD::setEntityNameStrings('partner', 'partnerek');
    }
    ...
    protected function setupCreateOperation()
    {
        ...
        CRUD::field('name')->label('Partner neve')->tab('Alapadatok');
    ...
    {
        $partnerId = request()->route('id') ?? 'new';
        $contacts = $partnerId !== 'new' ? PartnerContact::where('partner_id', $partnerId)->get() : collect();
        ...
        <div id="contacts-widget">
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="text-muted mb-0 mt-2">Itt adhatja meg a partner további kapcsolattartóit (pl. feleség, családtagok, stb.).</p>
                </div>
                <div class="card-body">
                    <!-- Add Contact Form -->
                    <div class="mb-4">
    ...
    {
        $partnerId = request()->route('id') ?? 'new';
        $documents = $partnerId !== 'new' ? PartnerDocument::where('partner_id', $partnerId)->get() : collect();
     ...
     */
    public function addContact(Request $request, $partnerId)
    {
        try {
            $request->validate([
     ...
     */
    public function uploadDocument(Request $request, $partnerId)
    {
        try {
            \Log::info('Partner document upload attempt', [
                'partner_id' => $partnerId,
            ...
            // Store file in partner documents directory
